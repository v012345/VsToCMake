	...
	namespace Cells {
		namespace Systems {
			...
			public:
				...
				static Single Parse(StringPtr s, intrusive_ptr<CultureInfo> provider);

				static bool TryParse(StringPtr s, Single* result);

				static bool TryParse(StringPtr s, NumberStyles style, intrusive_ptr<IFormatProvider> provider, Single* result);
				...
				static StringPtr ToString(Single self, intrusive_ptr<CultureInfo> provider);
				static StringPtr ToString(ObjectPtr obj);
			};
		}
	}
}
